...
import type { DupliosUserProfile, UserRole } from "../types/tenant";
...
const navGroups: Array<{
...
}> = [
  {
    ...
    items: [
      ...
      { id: "growth-crm", labelKey: "nav.leads", icon: Users, modes: ["growing", "leader"] },
      { id: "tasks", labelKey: "nav.followUp", icon: CheckSquare, modes: ["growing", "leader"] },
      { id: "events", labelKey: "nav.events", icon: CalendarDays, modes: ["growing", "leader"] },
      { id: "ai-scripts", labelKey: "nav.scripts", icon: MessageSquare, modes: ["growing", "leader"] }
    ]
  },
  {
    label: "Grow",
    ...
    items: [
      { id: "automation", labelKey: "nav.automation", icon: Bolt, modes: ["growing", "leader"] },
      { id: "team-hub", labelKey: "nav.team", icon: Users, modes: ["leader"] },
      { id: "duplication", labelKey: "nav.duplication", icon: Library, modes: ["leader"] },
      { id: "resource-vault", labelKey: "nav.content", icon: BookOpen, modes: ["new_joiner", "growing", "leader"] },
      { id: "team-metrics", labelKey: "nav.reports", icon: BarChart3, modes: ["leader"] }
    ]
  },
  {
    label: "Scale",
...
const roleNavGroups: Record<Exclude<UserRole, "superadmin">, Array<{
  label: string;
  items: Array<{ id: string; labelText: string; icon: typeof Home }>;
}>> = {
  new_joiner: [
    {
      label: "Member",
      items: [
        { id: "dashboard", labelText: "Today", icon: Home },
        { id: "growth-crm", labelText: "Leads", icon: Users },
        { id: "tasks", labelText: "Follow-Up", icon: CheckSquare },
        { id: "ai-scripts", labelText: "Scripts", icon: MessageSquare },
        { id: "events", labelText: "Events", icon: CalendarDays },
        { id: "settings", labelText: "Help", icon: Settings }
      ]
    }
  ],
  member: [
  ...
  leader: [
    {
      label: "Leader",
      items: [
        { id: "dashboard", labelText: "Team Dashboard", icon: Home },
        { id: "team-hub", labelText: "Members", icon: Users },
        { id: "team-metrics", labelText: "Follow-Up Health", icon: Gauge },
        { id: "events", labelText: "Events", icon: CalendarDays },
        { id: "playbook", labelText: "Playbooks", icon: Target },
        { id: "team-metrics", labelText: "Reports", icon: BarChart3 }
      ]
    }
  ],
  admin: [
    {
      label: "Admin",
      items: [
        { id: "dashboard", labelText: "Dashboard", icon: Home },
        { id: "automation", labelText: "Campaigns", icon: Bolt },
        { id: "ai-scripts", labelText: "Scripts", icon: MessageSquare },
        { id: "playbook", labelText: "Playbooks", icon: Target },
        { id: "resource-vault", labelText: "Resource Vault", icon: BookOpen },
        { id: "team-hub", labelText: "Team", icon: Users },
        { id: "legal", labelText: "Compliance", icon: Settings },
        { id: "team-metrics", labelText: "Reports", icon: BarChart3 },
        { id: "settings", labelText: "Settings", icon: Settings }
      ]
    }
  ]
};
...
export function AppShell({
  ...
  const role = profile?.isSuperadmin ? "superadmin" : profile?.role;
  const visibleGroups = role && role !== "superadmin"
    ? roleNavGroups[role]
    : navGroups
  ...
  return (
    <main className={`app-theme-${theme} min-h-screen`}>
      <aside className="app-sidebar fixed inset-y-0 left-0 z-30 hidden w-[208px] overflow-hidden border-r lg:block">
        <div className="flex h-full flex-col overflow-y-auto overscroll-contain px-3 py-4">
            ...
            {visibleGroups.map((group) => (
              <div key={group.label}>
                <p className="app-muted mb-3 px-2 text-[10px] font-black uppercase tracking-[0.28em]">
                  {group.label === "Start" || group.label === "Grow" || group.label === "Scale"
                ...
                <div className="space-y-1.5">
                  {group.items.map((item) => {
                    ...
                    return (
                      ...
                      >
                        <Icon size={17} />
                        <span className="flex-1">{"labelKey" in item ? translate(language, item.labelKey) : item.labelText}</span>
