...
export function DiamondPath() {
  ...
  const { diamondProspects, diamondCustomers, diamondFollowUps, addDiamondProspect, updateDiamondProspect, addDiamondFollowUp, updateDiamondFollowUp } = useDataStore();
  const [tab, setTab] = useState<"prospects" | "customers" | "followups">("prospects");
  ...
  return <section className="space-y-6">
    <div className="flex flex-wrap items-start justify-between gap-4">
      ...
      <button type="button" onClick={() => setShowForm(value => !value)} className="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-700">Add prospect</button>
    </div>
    <div className="grid gap-3 sm:grid-cols-3"><Metric label="Active prospects" value={diamondProspects.filter(p => !["Archived", "Not Interested"].includes(p.status)).length} /><Metric label="Customers" value={diamondCustomers.length} /><Metric label="Open follow-ups" value={dueFollowUps.length} /></div>
