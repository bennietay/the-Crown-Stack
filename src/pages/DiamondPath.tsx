...
export function DiamondPath() {
  ...
  return <section className="space-y-6">
    ...
    <div className="grid gap-3 sm:grid-cols-3 lg:grid-cols-6"><Metric label="Active prospects" value={diamondProspects.filter(p => !["Archived", "Not Interested"].includes(p.status)).length} /><Metric label="Customers" value={diamondCustomers.length} /><Metric label="Open follow-ups" value={dueFollowUps.length} /><Metric label="Products" value={diamondProducts.length}/><Metric label="Orders" value={diamondPurchases.length}/><Metric label="PV recorded" value={diamondPurchases.filter(p => p.status !== "Cancelled").reduce((sum, p) => sum + Number(p.pv || 0), 0)}/></div>
