...
export function Dashboard() {
  ...
  const { leads, opportunities, proposals, tasks, customers, tickets, revenueEvents, revenueGoals, notifications, diamondProspects, diamondCustomers, diamondFollowUps, diamondPurchases } = useDataStore();
  ...
  return (
    <div className="p-8 space-y-8 flex-1 overflow-y-auto">
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card><CardHeader className="pb-2"><CardTitle className="text-sm">Next 7 days</CardTitle></CardHeader><CardContent><p className="text-2xl font-extrabold text-slate-900">MYR {Math.round(monthRevenue.expected + pipelineValue * 0.25).toLocaleString()}</p><p className="mt-1 text-xs text-slate-500">Expected from booked revenue and 25% of active pipeline</p></CardContent></Card>
      </div>

      <div className="grid gap-4 lg:grid-cols-[1.2fr_1fr]">
        <Card className="lg:col-span-2"><CardHeader><CardTitle className="text-lg">Business performance</CardTitle><p className="text-xs text-slate-500">Portfolio summary only. Select a workspace to work its pipeline and follow-ups.</p></CardHeader><CardContent className="grid gap-3 md:grid-cols-2"><PerformanceRow label="WAAS" value={`MYR ${summarizeRevenue(waasRevenue).collected.toLocaleString()}`} detail={`${wLeads.length} leads · ${wOpps.length} opportunities · ${wonThisMonthMRC.toLocaleString()} MRR`} /><PerformanceRow label="Amway" value={`MYR ${summarizeRevenue(amwayRevenue).collected.toLocaleString()}`} detail={`${diamondProspects.length} prospects · ${diamondCustomers.length} customers · ${diamondPurchases.length} purchases`} /></CardContent></Card>
      </div>
      ...
      <div className="grid gap-4 xl:grid-cols-[1.35fr_1fr]">
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-4"><h3 className="font-bold text-slate-900">Portfolio scope</h3><p className="mt-1 text-xs text-slate-500">Operational tasks, pipelines and customer records are available only inside their selected workspace.</p><div className="mt-3 rounded-lg bg-slate-50 p-3 text-xs text-slate-600">Use the Active Workspace selector to open WAAS or Amway execution views.</div></div>
      </div>
      {/* Portfolio metrics intentionally stop at summary-level data. */}
...
function PerformanceRow({ label, value, detail }: { label: string; value: string; detail: string }) {
  return <div className="block rounded-lg border border-slate-100 bg-slate-50 p-3"><div className="flex items-center justify-between"><span className="text-sm font-bold text-slate-900">{label}</span><span className="text-sm font-extrabold text-emerald-700">{value}</span></div><p className="mt-1 text-xs text-slate-500">{detail}</p></div>;
}
