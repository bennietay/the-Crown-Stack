...
export function FastStartOS({
  ...
  const plan = fastStartPlan.length ? fastStartPlan : defaultThirtyDayFastStart;
  const [activeDay, setActiveDay] = useState(plan[0]?.day ?? 1);
  const [copied, setCopied] = useState<string | null>(null);
  const currentPlan = plan.find((item) => item.day === activeDay) ?? plan[0];
...
const defaultThirtyDayFastStart: FastStartPlan[] = Array.from({ length: 30 }, (_, index) => {
  const day = index + 1;
  const milestones: Record<number, FastStartPlan> = {
    1: {
      day,
      title: "Add your 30-name list",
      objective: "Build your first relationship list without judging who is ready.",
      actions: ["Add 30 names", "Tag product/business/event interest", "Ask sponsor to review your list"]
    },
    2: {
      day,
      title: "Send first 10 warm messages",
      objective: "Start conversations manually and compliantly.",
      actions: ["Copy approved warm message", "Open WhatsApp for 10 people", "Log every reply"]
    },
    3: {
      day,
      title: "Log replies and schedule follow-ups",
      objective: "Turn replies into the next best action.",
      actions: ["Log interested replies", "Schedule busy/not-now follow-ups", "Ask sponsor for help on hard replies"]
    },
    4: {
      day,
      title: "Invite prospects to overview/event",
      objective: "Move warm conversations into a simple presentation.",
      actions: ["Invite 5 prospects", "Create or join event", "Send calendar reminder"]
    },
    5: {
      day,
      title: "Learn objection handling",
      objective: "Answer common questions without pressure or claims.",
      actions: ["Review busy reply script", "Review price question script", "Save one sponsor-approved response"]
    },
    6: {
      day,
      title: "Book sponsor-assisted calls",
      objective: "Use sponsor support when the lead shows real interest.",
      actions: ["Identify 2 hot leads", "Request sponsor assist", "Book one short call"]
    },
    7: {
      day,
      title: "Review progress with sponsor",
      objective: "Use activity data to improve week two.",
      actions: ["Review leads added", "Review replies logged", "Set next 7-day target"]
    },
    14: {
      day,
      title: "Consistency checkpoint",
      objective: "Check if the member is duplicating the daily action loop.",
      actions: ["Review duplication score", "Recover overdue follow-ups", "Assign next playbook"]
    },
    30: {
      day,
      title: "Duplication checkpoint",
      objective: "Help the member teach the same process to another person.",
      actions: ["Review first 30 days", "Package best scripts", "Help first team member duplicate"]
    }
  };

  return milestones[day] ?? {
    day,
    title: `Daily duplication action ${day}`,
    objective: "Keep the loop simple: add leads, send messages, log replies, and schedule the next follow-up.",
    actions: ["Add 3 names", "Send 5 warm messages", "Complete overdue follow-ups", "Log replies", "Ask sponsor for one review if stuck"]
  };
});
