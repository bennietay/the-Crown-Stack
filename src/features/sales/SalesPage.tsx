...
export function SalesPage({ content, locationPricing = [], onStart, onCheckout }: SalesPageProps) {
  ...
  const safeContent = useMemo(
    () => ({
      headline: content.headline || "Duplios",
      subheadline: content.subheadline || "Relationship-first CRM, follow-up, and duplication workspace.",
      primaryCta: content.primaryCta || "Start 14-Day Free Trial",
      proofPoints: (content.proofPoints ?? []).filter((point): point is string => Boolean(point)),
      offerStack: (content.offerStack ?? []).filter((item): item is string => Boolean(item)),
      features: (content.features ?? []).filter(
        (feature): feature is SalesPageContent["features"][number] =>
          Boolean(feature?.title) && Boolean(feature?.body)
      ),
      pricing: (content.pricing ?? []).filter(
        (plan): plan is SalesPageContent["pricing"][number] =>
          Boolean(plan?.tier) && Boolean(plan?.name) && Boolean(plan?.price)
      ),
      testimonials: (content.testimonials ?? []).filter(
        (testimonial): testimonial is SalesPageContent["testimonials"][number] =>
          Boolean(testimonial?.quote) && Boolean(testimonial?.name)
      ),
      faqs: (content.faqs ?? []).filter(
        (faq): faq is SalesPageContent["faqs"][number] => Boolean(faq?.question) && Boolean(faq?.answer)
      )
    }),
    [content]
  );
  ...
  const checkout = (tier: SubscriptionTier) => {
    trackDupliosEvent("pricing_plan_click", { tier, countryCode: selectedLocationRule?.countryCode ?? detectedCountry });
    onCheckout(tier, buyerEmail, selectedLocationRule?.countryCode ?? detectedCountry);
  };

  const startTrial = () => {
    trackDupliosEvent("start_trial_click", { surface: "sales_page" });
    checkout("ascent");
  };

  const previewDashboard = () => {
    trackDupliosEvent("preview_dashboard_click", { surface: "sales_page" });
    onStart();
  };
  ...
  return (
    <div className="sales-motion overflow-hidden">
      <section className="sales-hero relative mx-auto grid min-h-[calc(100vh-73px)] max-w-7xl gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:px-8">
        ...
        <div className="motion-rise">
          <div className="glass-chip motion-badge mb-5 inline-flex items-center gap-2 rounded-full px-3 py-1 text-sm text-crown-champagne">
            ...
            Built for network marketers, direct sellers, and team leaders
          </div>
          ...
          <div className="motion-rise mt-4 flex flex-col gap-3 sm:flex-row">
            ...
            <button onClick={previewDashboard} className="motion-lift inline-flex h-12 items-center justify-center rounded-lg border border-white/10 px-5 font-semibold text-white transition hover:border-crown-gold/60">
              Preview dashboard
            </button>
          </div>

          <div className="motion-rise mt-6 grid gap-2 text-sm text-crown-mist sm:grid-cols-2">
            {safeContent.proofPoints.map((item) => (
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-3">
            {safeContent.proofPoints.map((point, index) => (
        ...
        <div className="glass-panel motion-float rounded-[24px] p-5 shadow-glow">
          ...
          <div className="space-y-3">
            {safeContent.offerStack.map((item, index) => (
              <div
                key={item}
      ...
      <section className="border-y border-white/10 bg-crown-ink/70 px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-8 max-w-4xl">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-crown-gold">The real leak</p>
            <h2 className="mt-2 text-3xl font-semibold text-white sm:text-4xl">
              Most network marketers do not lose because they lack leads. They lose because follow-up is messy.
            </h2>
          </div>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {[
              ["Prospects get forgotten", "Duplios turns every lead into a dated next action and recovery signal."],
              ["Conversations are scattered", "Trust notes, scripts, reminders, and customer context sit in one profile."],
              ["Event no-shows are wasted", "No-show lists and recovery scripts keep missed presentations alive."],
              ["Team members do not duplicate", "Playbooks, checklists, and shared scripts turn leader behavior into repeatable team action."],
              ["Customers are not reminded to reorder", "Customer profiles track product focus, purchase history, and next purchase windows."],
              ["Leaders cannot see activity clearly", "Reports show follow-up activity, event conversion, recovery opportunities, and team momentum."]
            ].map(([title, body]) => (
      ...
      <section className="px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <div className="mb-8 max-w-3xl">
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-crown-gold">Core Journey</p>
            <h2 className="mt-2 text-3xl font-semibold text-white sm:text-4xl">
              Capture, follow up, invite, recover, close, and duplicate.
            </h2>
            <p className="mt-4 text-sm leading-7 text-crown-mist">
              Duplios supports the full relationship-based selling cycle from the first warm conversation to customer care and team onboarding.
            </p>
          </div>
          <div className="grid gap-3 md:grid-cols-7">
            {["Capture", "Follow Up", "Invite", "Event", "Recover", "Close", "Duplicate"].map((step, index) => (
      ...
      <section className="px-4 py-14 sm:px-6 lg:px-8">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-2">
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-crown-gold">Who it is for</p>
            <h2 className="mt-2 text-3xl font-semibold text-white sm:text-4xl">
              Built for people who grow through conversations.
            </h2>
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              {["Network marketers", "Direct sellers", "Team leaders", "Social sellers", "Product educators", "Webinar / Zoom presenters"].map((item) => (
                <div key={item} className="rounded-lg border border-white/10 bg-white/[0.045] p-4 text-sm font-semibold text-crown-champagne">
                  {item}
                </div>
              ))}
            </div>
          ...
          <div>
            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-crown-gold">What Duplios is not</p>
            <h2 className="mt-2 text-3xl font-semibold text-white sm:text-4xl">
              Follow up like a human, not a spam bot.
            </h2>
            <div className="mt-6 space-y-3">
              {["Not a spam tool", "Not a mass-blast machine", "Not a guaranteed income system", "Not a replacement for real conversations", "Not tied to one network marketing company"].map((item) => (
                <div key={item} className="flex gap-3 rounded-lg border border-white/10 bg-crown-navy p-4 text-sm text-crown-mist">
                  <CheckCircle2 size={17} className="mt-0.5 shrink-0 text-crown-emerald" />
                  {item}
                </div>
              ))}
            </div>
            <p className="mt-5 text-sm leading-7 text-crown-mist">
              Duplios gives you structure, reminders, scripts, and visibility. The relationship still belongs to you.
            </p>
          </div>
        </div>
      </section>
      ...
      {safeContent.testimonials.length > 0 ? (
        <>
          ...
          <section className="mx-auto grid max-w-7xl gap-4 px-4 pb-12 sm:px-6 lg:grid-cols-3 lg:px-8">
            {safeContent.testimonials.map((testimonial) => (
      ...
      <section className="px-4 py-14 sm:px-6 lg:px-8">
        <div className="motion-glow mx-auto max-w-7xl rounded-lg border border-crown-gold/30 bg-crown-gold/10 p-6 text-center sm:p-10">
          ...
          <h2 className="text-3xl font-semibold text-white">Ready to stop losing warm prospects?</h2>
          <p className="mx-auto mt-3 max-w-2xl text-sm leading-7 text-crown-champagne">
            Start building a follow-up and duplication system your team can actually use.
          </p>
          <button
            onClick={startTrial}
          ...
          <p className="mx-auto mt-5 max-w-3xl text-xs leading-6 text-crown-mist">
            Duplios provides CRM organization, follow-up workflows, scripts, and team operating tools. It does not guarantee sales, income, rank advancement, customer acquisition, or business success.
          </p>
        </div>
      </section>
    </div>
