...
export type WaasStyle = "modern" | "bold" | "premium" | "minimal" | "elegant" | "vibrant";
...
export type WaasDiscountType = "percentage" | "fixed";
export interface WaasDeal { id: string; workspaceId: string; name: string; code: string; productType?: WaasProductType; planId?: string; discountType: WaasDiscountType; discountValue: number; startsAt?: string; endsAt?: string; maxRedemptions?: number; redemptions: number; active: boolean; createdAt: string; updatedAt: string; }
export interface WaasTemplate { id: string; workspaceId: string; name: string; productType: WaasProductType; niche: string; style: WaasStyle; version: string; previewImage?: string; demoUrl?: string; status: "draft" | "active" | "retired"; themePackageId: string; configuration: Record<string, unknown>; supportedSections: string[]; aiPromptProfile?: string; deploymentMetadata?: Record<string, unknown>; changelog?: string; createdAt: string; updatedAt: string; }
