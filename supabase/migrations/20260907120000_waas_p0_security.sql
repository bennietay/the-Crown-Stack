-- P0 security hardening: this function is server-only.  The service role is
-- used by the API after it has verified the workspace and website relation.
revoke all on function public.record_waas_update_usage(text,text,text,jsonb,integer) from public, anon, authenticated;
grant execute on function public.record_waas_update_usage(text,text,text,jsonb,integer) to service_role;

-- Do not allow a privileged caller to write usage against an unknown website.
-- This prevents malformed server requests from creating orphaned records.
...
begin
  select data into v_website
   ...
   where workspace_id = p_workspace_id
     and collection_name = 'waas_websites'
     and record_id = p_website_id
     and is_soft_deleted = false;
  if v_website is null then
    raise exception using errcode = 'P0002', message = 'Managed website not found';
  end if;
